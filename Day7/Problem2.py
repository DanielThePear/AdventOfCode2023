...
def identify_hand_type(hand):
    ...
    hand_counter = collections.Counter(hand).most_common(5)
    if hand_counter[0][0] == "J": hand_counter = hand_counter[1:]

    if j_count in range(1, 3): return identify_hand_type(hand.replace("J", hand_counter[0][0]))
    elif j_count in range(3, 5): return identify_hand_type(hand.replace("J", hand_counter[-1][0]))
    else: return 7
